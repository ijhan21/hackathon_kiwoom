...
class Kiwoom():
    ...
    def _handler_real_data(self, sCode, sRealType, sRealData):
#        
            pass
...
#           
    ...
    def request_account_balance(self, account):
        # xKHOpenAPI1.SetInputValue("계좌번호", accountNumber);
        ...
        self.CommRqData("계좌평가잔고내역요청", "opw00018", 0, "8100")

    def block_request_m(self, *args, **kwargs):
        trcode = args[0].lower()
        lines = parser.read_enc(trcode)
        self.tr_items= parser.parse_dat(trcode, lines)
        ...
        next = kwargs["next"]
        codes = kwargs["종목코드"]
        code_len = len(codes.split(";"))

        # set input
        for id in kwargs:
            if id.lower() != "output" and id.lower() != "next" and id !="종목코드":
                self.SetInputValue(id, kwargs[id])
        # initialize
        self.received = False
        self.tr_remained_m = False

        # request
        ret = self.CommKwRqData(codes, next, code_len,0,trcode,"0101")
        while not self.received:
            pythoncom.PumpWaitingMessages()            
...
#         self.app = app
#         self.kiwoom = Kiwoom()
#         self.kiwoom.SetRealReg(1,"005930","910","주문체결")
#         self.app.exec_()
